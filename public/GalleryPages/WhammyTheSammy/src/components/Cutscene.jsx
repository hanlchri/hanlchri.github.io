...
export default function Cutscene({ onTransition }) {
    const canvasRef = useRef(null);
    const { noiseBurst, stopDrone } = useAudio();
    const timeRef = useRef(0);

    useEffect(() => {
        noiseBurst(0.4, 0.09);
        stopDrone();
    }, [noiseBurst, stopDrone]);
    ...
    useGameLoop((dt) => {
        ...
        ctx.fillStyle = f > 0.5 ? '#83769c' : '#1a1a1a';
        ...
        for (let i = 0; i < 3; i++) {
            const ox = Math.sin((t + i) * 4) * 2 * (i + 1);
            const oy = Math.cos((t + i) * 3) * 2 * (i + 1);
            ctx.save();
